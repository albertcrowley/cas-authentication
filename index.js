 ...
 * @constructor
 */
function CASAuthentication(options, logger) {

    this._log = (msg, obj) => {
        if (logger) {
            let extra = Object.assign({tag: "CAS"}, obj || {} )
            logger.log("debug", msg, extra)
        }
    }

    this._log("created cas auth object")
    ...
    this._log(`using cas version ${this.cas_version}`)
...
CASAuthentication.prototype._handle = function(req, res, next, authType) {
    ...
    this._log(`In _handle for ${authType}`, {authType: authType, req: req})
...
CASAuthentication.prototype._login = function(req, res, next) {
    ...
    query[this.renew_query_parameter_name] = this.renew;
